...
def student_absence_keyboard(
    ...
    if selected_button == State.PRESENT.value:
        ...
        present_text, absent_text, late_text, le_text, il_text = base_text[
            selected_button
        ]
    ...
    absent = InlineKeyboardButton(
        absent_text, callback_data=f"{State.LA.value} {student_id}"
    )
    ...
    if selected_button != State.PRESENT.value:
        late = InlineKeyboardButton(
            late_text, callback_data=f"{State.LA.value} {student_id}"
        )
        le = InlineKeyboardButton(
            le_text, callback_data=f"{State.LE.value} {student_id}"
        )
        il = InlineKeyboardButton(
            il_text, callback_data=f"{State.IL.value} {student_id}"
        )
