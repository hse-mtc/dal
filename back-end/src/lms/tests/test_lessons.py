...
@pytest.mark.django_db
def test_get_by_id(get_new_lesson_data, su_client, create_lesson):
    ...
    lesson_responce_get = su_client.get(f"/api/lms/lessons/{lesson.id}/")

    assert lesson_responce_get.status_code == 200

    lesson_responce_get = lesson_responce_get.json()

    lesson_responce_get["milgroup"] = lesson_responce_get["milgroup"]["id"]
    lesson_responce_get["subject"] = lesson_responce_get["subject"]["id"]
    lesson_responce_get["room"] = lesson_responce_get["room"]["id"]

    assert lesson_responce_get == lesson_data


@pytest.mark.django_db
def test_put(get_new_lesson_data, su_client, create_lesson):
    lesson_data = get_new_lesson_data(ids_only=False)
    lesson = create_lesson(**lesson_data)
    ...
    assert lesson_responce_put.data["type"] != lesson.type
    assert lesson_responce_put.data["ordinal"] - 1 == lesson.ordinal
...
def test_patch(
    ...
    lesson_responce_patch = su_client.patch(
...
@pytest.mark.django_db
def test_delete(su_client, get_new_lesson_data, create_lesson):
    lesson_data = get_new_lesson_data(ids_only=False)
    first_lesson_data = create_lesson(**lesson_data)
