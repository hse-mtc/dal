...
class PaperFilter(filters.FilterSet):
    ...
    extra_filter = filters.CharFilter(method="filter_json")
    ...
    def filter_json(self, queryset, name, value):
        ...
        json_field = "data"  # TODO: заменить на имя JSONField
        built = self._build_q_from_token(value, json_field)
        if isinstance(built, tuple):
            q, annotations = built
            if annotations:
                queryset = queryset.annotate(**annotations)
            return queryset.filter(q)
        return queryset.filter(built)
