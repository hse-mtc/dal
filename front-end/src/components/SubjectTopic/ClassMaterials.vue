  ...
  <div class="class-materials file-group">
    ...
    <div style="display: none">
      <div class="files" v-if="hasMaterials">
        <div class="file" v-for="(material, index) in displayMaterials" :key="index">
          <div class="file-icon">
            <img
                :src="require(`../../assets/extensions-icon/${Object.keys(icons).includes(material.file.extension) ? icons[material.file.extension] : 'new-file'}.svg`)"
                alt=""
                class="kebab"
                width="48px"
                height="48px"
    ...
    >
      <el-upload
          ...
          :multiple="true"
      >
        <el-button class="upload-button" size="small" type="primary" style="outline: none">Добавить файл
        </el-button>
...
export default {
  ...
  methods: {
    icon(extension) {
      return Object.keys(this.icons).includes(extension) ? this.icons[extension] : '../../assets/extensions-icon/new-file.svg'
    },
    cancelFile() {
      this.fileList = []
    ...
    deleteMaterial(id) {
      this.$confirm(
        ...
        deleteMaterial(id).then(() => {
          this.displayMaterials = this.displayMaterials.filter((item) => item.id !== id);
        });
      });
    },
    handleClose(done) {
