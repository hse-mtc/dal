<template>
  <el-row class="register" type="flex">
    <el-col :xl="12" :lg="12" class="register-col">
      ...
      >
        <div>Ожидайте подтверждения учетной записи. После подтверждения на почту будет выслана ссылка для создания
          пароля.
        </div>
      </el-dialog>
    </el-col>
    <el-col :span="12" class="hidden-md-and-down register-col register-col-last"/>
  </el-row>
</template>

<script>
import {validCorEmail} from '@/utils/validate'
import {getMilFaculties, getMilGroups, getPosts, getRanks} from "@/api/reference-book";
  ...
  computed: {
    disabledButton() {
      if (this.registerForm.role === 'student') {
        return !this.registerForm.role || !this.registerForm.milgroup || !this.registerForm.email
      }
